...
def get_accuracy(model: torch.nn.Module,
                ...
                epoch: int = None, corruption: str = None, severity: int = None):
    if device is None:
    ...
    # 构建描述信息
    desc = f"Epoch {epoch+1}/{cfg.TEST.EPOCH}" if epoch is not None else "Evaluating"
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(pbar):
            samples += len(data[0])
            imgs, labels = data[0], data[1].to(device)
            # 处理图像，适应模型输入
            ...
            if isinstance(output, tuple):
                ...
                ood_score = -entropy  # 假设 OOD 分数为负熵值
            ...
            acc_current = (preds == labels).float().mean().item()
             # 更新进度条的后缀信息
            pbar.set_postfix({'Acc': f"{acc_current:.2%}"})
